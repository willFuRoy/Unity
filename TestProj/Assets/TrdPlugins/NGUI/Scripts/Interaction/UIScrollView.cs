...
public class UIScrollView : MonoBehaviour
{
    ...
    public OnDragNotification OnDraging;
	...
	/// <summary>
	/// Whether drag finished then back to top.
	/// </summary>
	public bool backWhenFits = false;
	...
	/// <summary>
	/// Event callback to trigger when the scroll view's movement ends and back to top.
	/// </summary>

	public OnDragNotification onBackToTopFinished;

	/// <summary>
	/// Event callback to trigger when the scroll move to last position;
	/// </summary>

	public OnDragNotification onMoveBackToLastPosition;
	...
	protected Vector3 mAwakePos = new Vector3();
	protected Vector3 mConstraint;

    private SpringPanel sp = null;
	...
	public virtual Bounds bounds
	{
		get
		{
			if (!mCalculatedBounds)
			{
				...
				//By Zsy : 根据策划需求，如果一个滑动面板内容不足以覆盖整个面板，且可以拖动内容，需要在松手后使其弹回顶端，backWhenFits若为true就表示需要弹回顶端
				if (!disableDragIfFits) {
					Vector4 clip = panel.finalClipRegion;
					backWhenFits = ((mBounds.size.x < clip.z) && canMoveHorizontally) || ((mBounds.size.y < clip.w) && canMoveVertically);
				}
            }
			return mBounds;
		}
	...
	protected virtual bool shouldMove
	{
		get
		{
			...
			{
				if (b.min.x < clip.x - hx - 1) return true;
				if (b.max.x > clip.x + hx + 1) return true;
			}

			if (canMoveVertically)
			{
				if (b.min.y < clip.y - hy - 1) return true;
				if (b.max.y > clip.y + hy + 1) return true;
			}
			return false;
		}
	}
	...
	public bool RestrictWithinBounds (bool instant, bool horizontal, bool vertical)
	{
       ...
       // Debug.LogError("Reset 1" + constraint);
        ...
        if (isRight) mConstraint.x = 1;
        if (isLeft) mConstraint.x = -1;
        if (isTop) mConstraint.y = 1;
        if (isBottom) mConstraint.y = -1;
        if (!horizontal) mConstraint.x = 0f;
        if (!vertical) mConstraint.y = 0f;
...
//		sp = GetComponent<SpringPanel>();
	...
	//By Zsy : 此处计算需要弹回顶端时具体偏移量
	Vector3 GetMoveOffsets()
	{
		mCalculatedBounds = false;
		...
		if (canMoveHorizontally) offsets.x = clip.x - ox;
		if (canMoveVertically) offsets.y = clip.y - oy;
	...
	public void ResetPosition()
	{
		if (NGUITools.GetActive(this))
		{
			...
			mAwakePos = mTrans.localPosition;
		}
	}

	//By Zsy : 参考resetpostion，这里把y坐标改为pv.y对应于1 - pv.y即为到底部
	[ContextMenu("Move To End Of List")]
	public void MoveToEnd()
	{
		if (NGUITools.GetActive(this))
		{
			...
			//// First move the position back to where it would be if the scroll bars got reset to zero
			SetDragAmount(1 - pv.x, pv.y, false);

			// Next move the clipping area back and update the scroll bars
			SetDragAmount(1 - pv.x, pv.y, true);
		}
	}
	...
	/// <summary>
	/// Back The View to Top of List
	/// </summary>

	public void BackToTopWhileFits()
	{
		if (sp == null)
			sp = gameObject.AddComponent<SpringPanel> ();

		if (sp != null) {
			SpringPanel.Begin (gameObject, panel.cachedTransform.localPosition + GetMoveOffsets(), 8, sp);
		}
	}
	...
	public void MoveStaticly(Vector3 relative)
	{
		mTrans.localPosition += relative;
		co = mPanel.clipOffset;
		co.x -= relative.x;
	...
	{
        point_a = mTrans.InverseTransformPoint(absolute);
        point_b = mTrans.InverseTransformPoint(Vector3.zero);
        MoveRelative(point_a - point_b);
	}
    ...
    float delta;
	Vector3 transdir = new Vector3();
	void LateUpdate ()
	{
		if (!Application.isPlaying) return;
		delta = RealTime.deltaTime;
		//Debug.LogError (bounds.size);
		...
		if (!mPressed)
		{
			if (mMomentum.magnitude > 0.0001f || mScroll != 0f)
			{
				if (movement == Movement.Horizontal)
				{
					transdir.Set (mScroll * 0.05f, 0f, 0f);
					mMomentum -= mTrans.TransformDirection(transdir);
				}
				else if (movement == Movement.Vertical)
				{
					transdir.Set (0f, mScroll * 0.05f, 0f);
					mMomentum -= mTrans.TransformDirection(transdir);
				}
				else if (movement == Movement.Unrestricted)
				{
					transdir.Set (mScroll * 0.05f, mScroll * 0.05f, 0f);
					mMomentum -= mTrans.TransformDirection(transdir);
				}
				else
				{
					transdir.Set (mScroll * customMovement.x * 0.05f, mScroll * customMovement.y * 0.05f, 0f);
					mMomentum -= mTrans.TransformDirection(transdir);
				}
    ...
    public void ResetSpring()
    {
        if (sp != null)
        {
            sp.target = transform.localPosition;
        }
    }
    ...
    public bool _isTop {
        get {
			if (canMoveHorizontally)
				return mConstraint.x > 0;
			else
				return mConstraint.y > 0;
        }
    }
    public bool _isDown {
        get {
    ...
    public void _MoveToTop() {
		ResetPosition ();
    }
    public void _MoveToDown() {
		MoveToEnd ();
		mConstraint.Set (-1, -1, 0);
        //Debug.LogError("Down" + mConstraint);
    }
	public void _MoveToSel(Transform tra) {
		Bounds wbounds = NGUIMath.CalculateRelativeWidgetBounds (mTrans, tra);
		...
		if (canMoveHorizontally) {
			float xmin = wbounds.min.x - mBounds.min.x;
			float xmax = mBounds.max.x - wbounds.max.x;
			float offset = 0;
			float rest = (maxArea.x - minArea.x) - wbounds.size.x;
			offset = xmax <= rest ? -(xmin - (rest - xmax)) : -xmin;
			ResetPosition ();
			mConstraint.Set (xmax <= rest ? -1 : 0, mConstraint.y, 0);
            //Debug.LogError("Sel " + mConstraint);
            MoveStaticly (new Vector3 (offset, 0, 0));
		} else if (canMoveVertically) {
			float ymin = wbounds.min.y - mBounds.min.y;
			float ymax = mBounds.max.y - wbounds.max.y;
			float offset = 0;
			float rest = (maxArea.y - minArea.y) - wbounds.size.y;
			offset = ymin <= rest ? (ymax - (rest - ymin)) : ymax;
			ResetPosition ();
			mConstraint.Set (mConstraint.x, ymin <= rest ? -1 : 0, 0);
            //Debug.LogError("Sel " + mConstraint);
			MoveStaticly (new Vector3 (0, offset, 0));
		}
    }
    #endregion
}
