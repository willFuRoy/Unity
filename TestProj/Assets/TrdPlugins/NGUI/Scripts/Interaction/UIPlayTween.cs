...
public class UIPlayTween : MonoBehaviour
{
    ...
    public GameObject[] customTweens;
	...
	public void Play (bool forward)
	{
        ...
        // llm 18-03-11 自定义 tweens
        if (customTweens != null && customTweens.Length > 0) {
            List<UITweener> ts = new List<UITweener>();
            for (int i = 0; i < customTweens.Length; ++i) {
                GameObject tgo = customTweens[i];
                if (tgo != null) {
                    ts.AddRange(tgo.GetComponents<UITweener>());
                }
            }
    ...
    public float playTime {
        get {
            if (mTweens == null) return 0.0f;

            float maxTime = float.MinValue;
            for(int i = 0, max = mTweens.Length; i < max; ++i) {
                maxTime = Mathf.Max(maxTime, mTweens[i].totalTime);
            }
