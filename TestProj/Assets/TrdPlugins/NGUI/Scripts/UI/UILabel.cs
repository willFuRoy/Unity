...
public class UILabel : UIWidget
{
	...
	[HideInInspector][SerializeField] UILabelBitMap lblBitMap;
    ...
    public static System.Func<UILabel, Vector3> onGetEmotionedLabelSizeEvt;

    Vector2 CalculateSize(string txt, UIFont font) {
        if (lblBitMap != null) {
            //return EmotionService.GetSizeOfEmotionedLabel(this);
            return onGetEmotionedLabelSizeEvt(this);
        }
        return NGUIText.CalculatePrintedSize(mProcessedText, font);
    }
	...
	void ProcessText (bool legacyMode, bool full)
	{
		...
		if (mFinalFontSize > 0)
		{
			...
			for (int ps = mFinalFontSize; ps > 0; --ps)
			{
                ...
                bool fits = true;
				// Wrap the text
                if (!mbFroce || (mText != mProcessedText && mbFroce))
				    fits = NGUIText.WrapText(mText, out mProcessedText, true, false, mOverflowEllipsis && mOverflow == Overflow.ClampContent);
				...
				else if (mOverflow == Overflow.ResizeFreely)
				{
                    ...
                    mCalculatedSize = CalculateSize(mProcessedText, null);
				...
				else if (mOverflow == Overflow.ResizeHeight)
				{
					...
					if (lblBitMap != null) {
						...
						bitfont = lblBitMap.bitMapFont;
					}
					//mCalculatedSize = NGUIText.CalculatePrintedSize(mProcessedText, bitfont);
                    mCalculatedSize = CalculateSize(mProcessedText, bitfont);
                    mHeight = Mathf.Max(minHeight, Mathf.RoundToInt(mCalculatedSize.y));
					if (regionY != 1f) mHeight = Mathf.RoundToInt(mHeight / regionY);
					if ((mHeight & 1) == 1) ++mHeight;
				}
	...
	public int GetCharacterIndexAtPosition (Vector2 localPos, bool precise)
	{
		if (isValid)
		{
			...
			if (precise) NGUIText.PrintExactCharacterPositions(text, mTempVerts, mTempIndices, lblBitMap);
	...
	public override void OnFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		NGUIText.tint = col;
        // By Zsy : lblBitMap是从脚本传进来的表情bitmap字体
        // By llm : 清理表情
        if (lblBitMap != null) lblBitMap.Clear();
		NGUIText.Print(text, verts, uvs, cols, lblBitMap);
		NGUIText.bitmapFont = null;
		NGUIText.dynamicFont = null;
    ...
    bool mbFroce;
	public void SetText(string _text, bool bFroce = false)
    {
        mbFroce = bFroce;
        if (bFroce)
        {
    ...
    public static void ClearUnvalidRef() {
        // static member
        //mFontUsage.ClearUnvalidRefKey();
        //mTempDrawcalls.ClearUnvalidRefValue();
    }
}
