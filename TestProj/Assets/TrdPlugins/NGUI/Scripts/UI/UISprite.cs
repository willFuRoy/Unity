...
public class UISprite : UIBasicSprite
{
	...
	public override Vector4 drawingDimensions
	{
		get
		{
            ...
            // llm 解决因 pading 导致不居中的问题
            if (mType == Type.Simple) {
                switch (mMirror) {
                    case Mirror.Horizontally:
                        vx -= mSprite.paddingLeft * pixelSize;
                        vz -= mSprite.paddingLeft * pixelSize;
                        break;
                    case Mirror.Vertically:
                        vy -= mSprite.paddingTop * pixelSize;
                        vw -= mSprite.paddingTop * pixelSize;
                        break;
                }
            }
	...
	static Rect gSpOuter = new Rect ();
	static Rect gSpInner = new Rect();
	public override void OnFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
        ...
        // llm 处理镜像问题，不通过配置 atlas_cfg.json 去解决镜像问题，通过该配置会引起其他图集的计算错误问题
        gSpOuter.Set(mSprite.x, mSprite.y, mSprite.width, mSprite.height);
        gSpInner.Set(mSprite.x + mSprite.borderLeft, mSprite.y + mSprite.borderTop,
            mSprite.width - mSprite.borderLeft - mSprite.borderRight,
            mSprite.height - mSprite.borderBottom - mSprite.borderTop);

        gSpOuter = NGUIMath.ConvertToTexCoords(gSpOuter, tex.width, tex.height);
		gSpInner = NGUIMath.ConvertToTexCoords(gSpInner, tex.width, tex.height);

		int offset = verts.size;
		Fill(verts, uvs, cols, gSpOuter, gSpInner);

		if (onPostFill != null)
			onPostFill(this, offset, verts, uvs, cols);
	}

	[ContextMenu("SetFillGeometry")]
	void SetGeometry()
	{
