...
public class UIPanel : UIRect
{
	...
	/// <summary>
	/// Just Make It True While the widgets of this Panel is often changed
	/// if true will make uidrawcalls block-updategeometry,which is faster and little GC but use twice memery
	/// this is Add by Zsy 2017.9.26
	/// </summary>

	public bool OftenFilled = false;
	...
	static bool panelDirty = false;


	bool mFixedToStart = false;
	public bool FixedToStart
	{
		get { return mFixedToStart; }
		set { mFixedToStart = value; }
	}
	...
	Vector4 mfinalClipRegion = new Vector4();
	public Vector4 finalClipRegion
	{
		get
		{
			...
			{
				mfinalClipRegion.Set (mClipRange.x + mClipOffset.x, mClipRange.y + mClipOffset.y, size.x, size.y);
				return mfinalClipRegion;//new Vector4(mClipRange.x + mClipOffset.x, mClipRange.y + mClipOffset.y, size.x, size.y);
			}
			mfinalClipRegion.Set (0f, 0f, size.x, size.y);
			return mfinalClipRegion;//new Vector4(0f, 0f, size.x, size.y);
		}
	}
	...
	public override void Invalidate (bool includeChildren, bool onlyAlpha = false)
	{
		mAlphaFrameID = -1;
		base.Invalidate(includeChildren, onlyAlpha);
	}
	...
	void LateUpdate ()
	{
...
#endif
		{
			...
			if (panelDirty) {
                ReallyUpdateRenderQueue (mDirty);
				panelDirty = false;
			}
			gAlphaChanged = false;
			#if UNITY_EDITOR
	...
	public void MarkDirty(bool refreshAll = true)
	{
		panelDirty = true;
		mDirty = !refreshAll;
	}

	public void MarkDirtyImm(bool refreshAll = true)
	{
		ReallyUpdateRenderQueue (refreshAll);
	}
		
    void ReallyUpdateRenderQueue(bool updateall = true)
	{
        mDirty = !updateall;
		rq = 3000;
		...
		for (int i = 0, imax = list.Count; i < imax; ++i) {
			UIPanel mp = list [i];
			UpdateDrawCallQueue (mp);

            // 处理粒子系统在相邻两个 Panel 间的前后关系导致的顺序问题
            rq += 2;
		}
		mDirty = false;
	}

	void UpdateDrawCallQueue(UIPanel mp)
	{
		bool updating = (mp.Equals (this) && mDirty);
		if (mp.renderQueue == RenderQueue.Automatic) {
			mp.startingRenderQueue = rq;
			mp.UpdateDrawCalls (updating);
			rq += mp.drawCalls.Count;
		}
		else if (mp.renderQueue == RenderQueue.StartAt)
		{
			mp.UpdateDrawCalls(updating);
			if (mp.drawCalls.Count != 0)
				rq = Mathf.Max(rq, mp.startingRenderQueue + mp.drawCalls.Count);
		}
		else // Explicit
		{
			mp.UpdateDrawCalls(updating);
			if (mp.drawCalls.Count != 0)
				rq = Mathf.Max(rq, mp.startingRenderQueue + 1);
		}
	}
	...
	void UpdateSelf ()
	{
		...
		{
			dcCount = drawCalls.Count;
			for (int i = 0; i < dcCount; )
			{
				dc = drawCalls[i];
		...
		{
			mUpdateScroll = false;
            if(sv == null) sv = GetComponent<UIScrollView>();
	...
	void FillAllDrawCalls ()
	{
		...
		for (int i = 0; i < widgetCount; ++i)
		{
			...
			if (w.isVisible && w.hasVertices)
			{
				...
				if (mat != mt || tex != tx || sdr != sd || grey != gr)
				{
					if (dc != null && dc.needbuild)
					{
						drawCalls.Add(dc);
				...
				if (mat != null || sdr != null || tex != null)
				{
					if (dc == null)
					{
						dc = UIDrawCall.Create(this, mat, tex, sdr, grey);
...
//					else w.WriteToBuffers(dc.verts, dc.uvs, dc.cols, null, null);
	...
	{
//		Debug.LogError ("Fill Single DrawCall");
		if (dc != null)
		{
			dc.isDirty = false;
			int count = 0;
			...
			for (int i = 0; i < widgets.Count; )
			{
				...
				if (w.drawCall == dc)
				{
					if (w.isVisible && w.hasVertices)
					{
						...
						//by Zsy：这里每个uidrawcall里面我加了一个list记录需要构建的widget，这里不再马上writetobuffers
						dc.AddWidget (w);
//						if (generateNormals) w.WriteToBuffers(dc.verts, dc.uvs, dc.cols, dc.norms, dc.tans);
//						else w.WriteToBuffers(dc.verts, dc.uvs, dc.cols, null, null);
			...
			//by Zsy : 这里只要有一个widget需要build，在此处一口气创建所有widget的网格信息
			if (dc.needbuild)
			{
				dc.UpdateGeometry(count);
				dc.onRender = mOnRender;
	...
	void UpdateDrawCalls (bool isthis = false)
	{
		trans = cachedTransform;
        if (mDirty && !isthis)
            goto Here;
...
Here:
		if (usedForUI)
		{
			tparent = cachedTransform.parent;
			...
			if (tparent != null) pos = tparent.TransformPoint(pos);
			pos += drawCallOffset;
		}
		...
		for (int i = 0; i < drawCalls.Count; ++i)
		{
			...
			t = dc.cachedTransform;
			t.position = pos;
			t.rotation = trans.rotation;
			t.localScale = trans.lossyScale;
	...
	void UpdateWidgets()
	{
		...
		{
            int count = UIScrollView.list.size;
            for (int i = 0; i < count; ++i)
			{
				sv = UIScrollView.list[i];
        ...
        int cnt = widgets.Count;
		for (int i = 0, imax = widgets.Count; i < imax; ++i)
		{
            // 修复 UIWidget w = widgets[i]; 代码访问越界错误
            // 应是在该循环中有 Widget 被删除导致
            if (cnt != widgets.Count) {
#if UNITY_EDITOR
                Debug.LogErrorFormat(gameObject, "Count diff: {0} {1} {2}", i, cnt, widgets.Count);
#else
                if (Debug.isDebugBuild) {
                    Debug.LogErrorFormat(gameObject, "Count diff: {0}", gameObject);
                }
#endif
...
//#endif
            ...
            if (w.panel.Equals(this) && w.enabled)
			{
#if UNITY_EDITOR
	...
	Vector2 minRect = new Vector2();
	Vector2 maxRect = new Vector2();
	Vector2 minArea = new Vector2();
	Vector2 maxArea = new Vector2();
	public virtual Vector3 CalculateConstrainOffset (Vector2 min, Vector2 max)
	{
		...
		minRect.Set(min.x, min.y);
		maxRect.Set(max.x, max.y);
		minArea.Set(cr.x - offsetX, cr.y - offsetY);
		maxArea.Set(cr.x + offsetX, cr.y + offsetY);
		...
		return NGUIMath.ConstrainRect(minRect, maxRect, minArea, maxArea, mFixedToStart);
	}
    ...
    /// <param name="atBottom"></param>
    /// <param name="atLeft"></param>
    /// <param name="atRight"></param>
    public void CheckBorder(ref bool atTop, ref bool atBottom, ref bool atLeft, ref bool atRight) {
        atTop = Mathf.Abs(maxRect.y - maxArea.y) <= 5.0f;
        atBottom = Mathf.Abs(minRect.y - minArea.y) <= 5.0f;

        atLeft = Mathf.Abs(minRect.x - minArea.x) <= 5.0f;
        atRight = Mathf.Abs(maxRect.x - maxArea.x) <= 5.0f;
    }
	...
	Vector2 mViewSize = new Vector2();
	public Vector2 GetViewSize ()
	{
		if (mClipping != UIDrawCall.Clipping.None) {
			mViewSize.Set(mClipRange.z, mClipRange.w);
			return mViewSize;//new Vector2 (mClipRange.z, mClipRange.w);
		}
    ...
    #region llm 粒子系统UI剪裁计算，在PanelModel 中进行计算

    public bool ignoreClip;
    [System.NonSerialized]
    public Vector4 particleClipRange;
    [System.NonSerialized]
    public Vector2 particleClipArgs = new Vector2(1000.0f, 1000.0f);
    public bool particleNeedClip { get { return clipping == UIDrawCall.Clipping.SoftClip; } }
