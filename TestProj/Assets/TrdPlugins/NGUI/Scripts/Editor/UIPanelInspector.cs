...
public class UIPanelInspector : UIRectEditor
{
	...
	protected override bool ShouldDrawProperties ()
	{
		...
		if (NGUIEditorTools.DrawHeader("Advanced Options"))
		{
			...
			GUILayout.BeginHorizontal();
			bool fixedtostart = EditorGUILayout.Toggle("Fit To Start", mPanel.FixedToStart, GUILayout.Width(100f));
			GUILayout.Label("When you want to move back if panel is fit");
			GUILayout.EndHorizontal();

			if (mPanel.FixedToStart != fixedtostart)
			{
				mPanel.FixedToStart = !mPanel.FixedToStart;
				EditorUtility.SetDirty(mPanel);
				EditorWindow.FocusWindowIfItsOpen<UIPanelTool>();
			}

            GUILayout.BeginHorizontal();
            bool ignoreClip = EditorGUILayout.Toggle("Ignore particles", mPanel.ignoreClip, GUILayout.Width(100f));
            GUILayout.Label("When you want to ignore clip particles");
            GUILayout.EndHorizontal();
            if (mPanel.ignoreClip != ignoreClip) {
                mPanel.ignoreClip = ignoreClip;
                EditorUtility.SetDirty(mPanel);
                EditorWindow.FocusWindowIfItsOpen<UIPanelTool>();
            }
