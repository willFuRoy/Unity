...
public class UIWidgetInspector : UIRectEditor
{
	...
	static public void DrawColor (SerializedObject so, UIWidget w)
	{
		if ((w.GetType() != typeof(UIWidget)))
		{
            ...
            #region llm  2018-8-25 灰度相关
            NGUIEditorTools.DrawProperty("GreyWeight", so, "mGreyWeight", GUILayout.MinWidth(20f));
            GUI.changed = false;
            bool grey = EditorGUILayout.Toggle("Grey", w.grey);
            if (GUI.changed) {
                w.grey = grey;
            }
            #endregion
        ...
        #region llm 2018-9-15 忽略Bounds计算
        NGUIEditorTools.DrawProperty("IgnoreBoundsCal", so, "mIgnoreBounds", GUILayout.MinWidth(20f));
        #endregion
    }
