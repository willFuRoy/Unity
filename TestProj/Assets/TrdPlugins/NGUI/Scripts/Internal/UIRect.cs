...
public abstract class UIRect : MonoBehaviour
{
	...
	[System.NonSerialized] protected bool mMoved = false;
	[System.NonSerialized] protected bool mParentFound = false;
	[System.NonSerialized] protected bool mAnchorTargetChanged = false;
	...
	[System.NonSerialized] const int mUpdateAnchorFrames = 1;
	//by Zsy : gAlphaChanged指当有任意一个uiwidget的alpha变化时为true，否则为false
	[System.NonSerialized] protected static bool gAlphaChanged = true;

	public static bool setDimension = false;
	...
	public virtual void Invalidate (bool includeChildren, bool onlyAlpha = false)
	{
		mChanged = mChanged ? mChanged : includeChildren;
		if (includeChildren || onlyAlpha)
			for (int i = 0; i < mChildren.size; ++i)
				mChildren.buffer[i].Invalidate(includeChildren, onlyAlpha);
	}
	...
	protected virtual void OnEnable ()
	{
		...
		if (updateAnchors == AnchorUpdate.OnEnable || updateAnchors == AnchorUpdate.OnChange)
		{
	...
	protected virtual void Awake ()
	{
		...
		updateAnchors = AnchorUpdate.OnChange;
    ...
    bool AnchorChanged(AnchorPoint ap) {
        return ap.rect.mAnchorTargetChanged || (ap.target != null && ap.target.hasChanged);
    }

	bool anchorTargetChanged = false;
	bool CheckChanging()
	{
        return AnchorChanged(leftAnchor)
            || AnchorChanged(rightAnchor)
            || AnchorChanged(topAnchor)
            || AnchorChanged(bottomAnchor);
	}
    ...
    public void Update ()
	{
...
#endif
		{
			...
			else if (updateAnchors == AnchorUpdate.OnChange) {
				//by Zsy : hasAnchorTarget表示是否有锚点挂载
				if (hasAnchorTarget) {
					//by Zsy : setDimension用于表示有一个rect有变化，CheckChanging()用于检测是否涉及自己
					if (CheckChanging() && setDimension) {
						UpdateAnchorsInternal (frame);	
					}
				}
			} 
	...
	{
		setDimension = false;
		if (hasAnchorTarget && anchorTargetChanged) {
			anchorTargetChanged = false;
			leftAnchor.rect.mAnchorTargetChanged = false;
		}
	}
