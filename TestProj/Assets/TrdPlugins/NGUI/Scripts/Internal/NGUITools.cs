...
//    }
	...
	static public UIPanel CreateUI (Transform trans, bool advanced3D, int layer)
	{
		...
		{
			for (int i = 0, max = UIRoot.list.Count; i < max; ++i)
			{
                UIRoot r = UIRoot.list[i];
...
//#endif
	...
	static public Vector3[] GetSides (this Camera cam, float depth, Transform relativeTo)
	{
...
#endif
		{
			...
			mSides [0].Set (x0, 0f, depth);
			mSides [1].Set (0f, y1, depth);
			mSides [2].Set (x1, 0f, depth);
			mSides [3].Set (0f, y0, depth);

			mSides[0] = rot * mSides [0] + pos;
			mSides[1] = rot * mSides [1] + pos;
			mSides[2] = rot * mSides [2] + pos;
			mSides[3] = rot * mSides [3] + pos;
		}
		else
		{
