...
public class UIGeometry
{
	...
	Vector4 tempRtpTan = Vector4.zero;
	public void ApplyTransform (Matrix4x4 widgetToPanel, bool generateNormals = true)
	{
		if (verts.size > 0)
		{
			mRtpVerts.Clear();
			int size = verts.size;
			for (int i = 0, imax = size; i < imax; ++i) mRtpVerts.Add(widgetToPanel.MultiplyPoint3x4(verts[i]));
			...
			if (generateNormals)
			{
				...
				tempRtpTan.x = tangent.x;
				tempRtpTan.y = tangent.y;
				tempRtpTan.z = tangent.z;
				tempRtpTan.w = -1f;
				mRtpTan = tempRtpTan;//new Vector4(tangent.x, tangent.y, tangent.z, -1f);
			}
		}
