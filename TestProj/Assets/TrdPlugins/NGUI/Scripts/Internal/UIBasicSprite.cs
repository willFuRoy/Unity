...
public abstract class UIBasicSprite : UIWidget
{
    ...
    // llm 镜像
    [HideInInspector][SerializeField] protected Mirror mMirror = Mirror.Nothing;
...
#region Fill Functions
	...
	Vector4 drawingUVs
	{
		get
		{
            ...
            switch (mFlip) {
                case Flip.Horizontally: gDrawingUVs.Set(mOuterUV.xMax, mOuterUV.yMin, mOuterUV.xMin, mOuterUV.yMax); break;
                case Flip.Vertically: gDrawingUVs.Set(mOuterUV.xMin, mOuterUV.yMax, mOuterUV.xMax, mOuterUV.yMin); break;
                case Flip.Both: gDrawingUVs.Set(mOuterUV.xMax, mOuterUV.yMax, mOuterUV.xMin, mOuterUV.yMin); break;
                default: gDrawingUVs.Set(mOuterUV.xMin, mOuterUV.yMin, mOuterUV.xMax, mOuterUV.yMax); break;
            }
            return gDrawingUVs;
        }
	}
    static Vector4 gDrawingUVs;

    /// <summary>
    /// 
    /// </summary>
    Vector4 drawingUVs_Inner {
        get {
            switch (mFlip) {
                case Flip.Horizontally: gDrawingUVs_Inner.Set(mInnerUV.xMax, mInnerUV.yMin, mInnerUV.xMin, mInnerUV.yMax); return gDrawingUVs_Inner;
                case Flip.Vertically: gDrawingUVs_Inner.Set(mInnerUV.xMin, mInnerUV.yMax, mInnerUV.xMax, mInnerUV.yMin); return gDrawingUVs_Inner;
                case Flip.Both: gDrawingUVs_Inner.Set(mInnerUV.xMax, mInnerUV.yMax, mInnerUV.xMin, mInnerUV.yMin); return gDrawingUVs_Inner;
                default: gDrawingUVs_Inner.Set(mInnerUV.xMin, mInnerUV.yMin, mInnerUV.xMax, mInnerUV.yMax); return gDrawingUVs_Inner;
            }
        }
    }
    static Vector4 gDrawingUVs_Inner;
	...
	/// <summary>
	/// Deal the mirror of sprite
	/// </summary>
	void FillMirrorVerticle(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols, Color32 c, float yindex = 1, float xindex = 1)
	{
		int vertsize = verts.size;
		Vector3 newvert;
		for (int i = 0; i < vertsize; ++i) {
			newvert = new Vector3(verts[i].x, verts[i].y, verts[i].z);
			newvert.y = verts [i].y + (verts [vertsize - 1].y + verts[0].y) * yindex;
			verts.Add (newvert);
		}

		float dis = (verts [vertsize - 1].y + verts [0].y) * yindex;
		...
		for (int i = 0; i < uvsize; ++i) {
			newuv = new Vector2(uvs[i].x, uvs[i].y);
			uvs.Add (newuv);
		}

		float y;
		for (int j = uvsize; j < uvsize * 2; ++j) {
			if (j % 2 == 0 && j < uvsize * 2 - 1) {
				y = uvs [j].y + uvs [j + 1].y;
				newuv = uvs [j + 1];
				newuv.y = y - uvs [j + 1].y;
				uvs [j + 1] = newuv;
	...
	void FillMirrorHorizon(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols, Color32 c, float yindex = 1, float xindex = 1)
	{
		int vertsize = verts.size;
		Vector3 newvert;
		for (int i = 0; i < vertsize; ++i) {
			newvert = new Vector3(verts[i].x, verts[i].y, verts[i].z);
			newvert.x = verts [i].x - (verts [vertsize - 1].x - verts [0].x) * 1;//xindex;
			verts.Add (newvert);
		}

		float dis = (verts [vertsize - 1].x - verts [0].x);
		for (int j = 0; j < vertsize * 2; ++j) {
			newvert = verts [j];
			newvert.x += dis * 0.5f;
			newvert.x *= 0.5f;
	...
	Vector3 Rotate1(Vector3 point, float angle)
	{
		angle = Mathf.PI / 180 * angle;
		Vector3 p = point;
		p.x = point.x * Mathf.Cos (angle) - Mathf.Sin (angle) * point.y;
		p.y = Mathf.Sin (angle) * point.x + Mathf.Cos (angle) * point.y;
		return p;
	}

	Vector3 Rotate2(Vector3 point, float angle)
	{
		angle = Mathf.PI / 180 * angle;
		Vector3 p = point;
		p.x = point.x * Mathf.Cos (angle) - Mathf.Sin (angle) * point.z;
		p.z = Mathf.Sin (angle) * point.x + Mathf.Cos (angle) * point.z;
		return p;
	}


	void FillMirrorSliced(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols, Color32 c)
	{
		if (mMirror.Equals (Mirror.Vertically)) {
			int vertsize = verts.size;
			Vector3 newvert;
			for (int i = 0; i < vertsize; ++i) {
				newvert = new Vector3 (verts [i].x, verts [i].y, verts [i].z);
				newvert.y = verts [i].y - (verts [vertsize - 2].y - verts [0].y);
				verts.Add (newvert);
			}
			...
			float dis = (verts [vertsize - 2].y - verts [0].y);
			for (int j = 0; j < vertsize * 2; ++j) {
				newvert = verts [j];
				newvert.y -= dis * 0.5f;
				newvert.y *= 0.5f;
			...
			for (int j = 0; j < vertsize; ++j) {
				newvert = verts [j];
				verts [j] = Rotate1 (newvert, 180);
			}
		} else if (mMirror.Equals (Mirror.Horizontally)) {
			int vertsize = verts.size;
			Vector3 newvert;
			for (int i = 0; i < vertsize; ++i) {
				newvert = new Vector3 (verts [i].x, verts [i].y, verts [i].z);
				newvert.x = verts [i].x - (verts [vertsize - 2].x - verts [0].x);
				verts.Add (newvert);
			}
			...
			float dis = (verts [vertsize - 2].x - verts [0].x);
			for (int j = 0; j < vertsize * 2; ++j) {
				newvert = verts [j];
				newvert.x -= dis * 0.5f;
				newvert.x *= 0.5f;
	...
	void SimpleFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		//if (mMirror.Equals (Mirror.Horizontally))
		//	FillMirrorHorizon (verts, uvs, cols, c);
		//else if (mMirror.Equals (Mirror.Vertically))
		//	FillMirrorVerticle (verts, uvs, cols, c);
	}

    /// <summary>
    /// llm  2018-1-22 镜像，使用同一个顶点
    /// </summary>
    /// <param name="verts"></param>
    /// <param name="uvs"></param>
    /// <param name="cols"></param>
    void SimpleFill_LLM(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols) {
        Vector4 v = drawingDimensions;
        Vector4 u = drawingUVs;
        Color32 c = drawingColor;
        ...
        switch (mMirror) {
            ...
            case Mirror.Horizontally:
                u = drawingUVs_Inner;
                Vector2 vMT = new Vector2((v.z + v.x) / 2.0f, v.w);
                Vector2 vMB = new Vector2(vMT.x, v.y);

                verts.Add(new Vector3(v.x, v.y));
                verts.Add(new Vector3(v.x, v.w));
                verts.Add(new Vector3(vMT.x, vMT.y));
                verts.Add(new Vector3(vMB.x, vMB.y));
            ...
            case Mirror.Vertically:
                u = drawingUVs_Inner;
                Vector2 vML = new Vector2(v.x, (v.w + v.y) / 2.0f);
                Vector2 vMR = new Vector2(v.z, vML.y);

                verts.Add(new Vector3(v.x, v.y));
                verts.Add(new Vector3(vML.x, vML.y));
                verts.Add(new Vector3(vMR.x, vMR.y));
                verts.Add(new Vector3(v.z, v.y));
    ...
    void SlicedFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		//FillMirrorSliced (verts, uvs, cols, c);
	}
	...
	void TiledFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		Vector4 realv = Vector4.zero;
		Vector4 realuv = Vector4.zero;

		float yindex = (v.w - y0) / size.y;
		float xindex = (v.z - x0) / size.x;
		...
		//if (mMirror.Equals (Mirror.Horizontally))
		//	FillMirrorHorizon (verts, uvs, cols, c, yindex, xindex);
		//else if (mMirror.Equals (Mirror.Vertically))
		//	FillMirrorVerticle (verts, uvs, cols, c, yindex, xindex);
	}
...
//		FillMirror (verts, uvs, cols, c);
	...
	protected override void FillAlpha ()
	{
		int count = geometry.cols.size;
		for (int i = 0; i < count; ++i)
			geometry.cols [i] = drawingColor;
	}
