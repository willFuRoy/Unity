...
//		if (i > 0 && IsSpace(s[i])) s[i] = '\n';
    ...
    public static Vector2 CalculatePrintedSize(UILabel label, string text)
    {
    ...
    public static void UpdateCharacterPosition(UILabel label, string str, out BetterList<Vector3> verts, out BetterList<int> indices)
    {
        //计算当前所有字符的位置  
        label.text = str;
	...
	//By Zsy : 图文混排后文字的顶点坐标发生变化，此处需要同步处理以保证URL解析能正确工作
	static public Vector3 CalculatePrintedSize (string text, UIFont SbitMapFont = null)
	{
		...
		if (!string.IsNullOrEmpty(text))
		{
			...
			//By Zsy : symbolY是计算出来的当前这一排最高图片的Y坐标，用于和原来的finalY比较并调整高度
			int symbolY = 0;
            float OrigfontScale = fontScale;
			for (int i = 0; i < textLength; ++i)
			{
				...
				if (symbol == null)
				{
                    ...
                    if (w != 0f)
					{
						...
						if (Mathf.RoundToInt(x + w) > regionWidth)
						{
							...
							x = w;
							y += (finalLineHeight > symbolY ? finalLineHeight : symbolY);
							symbolY = 0;
							yy = -1;
						}
						else x += w;
				...
				{
                    if(SbitMapFont != null)
                        fontScale = fontSize / (float)bitmapFont.defaultSize;
                    float w = finalSpacingX + symbol.advance * fontScale;

					symbolY = symbol.height + soffsetY > symbolY ? symbol.height + soffsetY : symbolY;
					if (yy != -1)
						yy1 = symbolY;
					if (Mathf.RoundToInt(x + w) > regionWidth)
					{
			...
			v.y = (y + (finalLineHeight > symbolY ? finalLineHeight : symbolY));
			v.z = yy1;
		}
		return v;
	}
	...
	static List<int> vertsIndexList = new List<int> (50);
	//By Zsy : 5只是一个偏移量，用于让排版好看一点
	static int soffsetY = 5;
	static public void Print (string text, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols, UILabelBitMap bitmaplbl = null)
	{
		vertsIndexList.Clear ();
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (ch == '\n')
			{
				...
				x = 0;
				y += finalLineHeight + LastPicYoffset;
				LastPicYoffset = 0;
				prev = 0;
				continue;
			}
			...
			if (bitmaplbl != null) {
				useSymbols = true;
				bitmapFont = bitmaplbl.bitMapFont;
			}
			// See if there is a symbol matching this text
			BMSymbol symbol = useSymbols ? GetSymbol(text, i, textLength) : null;
			...
			if (symbol != null)
			{
				...
				if (Mathf.RoundToInt(x + symbol.advance * fontScale) > regionWidth)
				{
					...
					v0x -= x;
					v1x -= x;
					v0y -= finalLineHeight + LastPicYoffset;
					v1y -= finalLineHeight + LastPicYoffset;

					x = 0;
					y += finalLineHeight + LastPicYoffset;
					LastPicYoffset = 0;
					prevX = 0;
					if (LastPicYoffset != 0 && vertsIndexList.Count > 0) {
						for (int j = 0, jmax = vertsIndexList.Count; j < jmax; ++j) {
							Vector3 temp = verts [vertsIndexList [j]];
							temp.Set (temp.x, temp.y - LastPicYoffset - finalLineHeight + fontSize + soffsetY, temp.z);
							verts [vertsIndexList [j]] = temp;
						}
					}
					vertsIndexList.Clear ();
				}
				...
				if (bitmaplbl != null) {
					bitmaplbl.SetSpriteData (symbol, new Vector3(v0x, v1y , 0), emojiindex++);
					float Yoffset = symbol.height + soffsetY > finalLineHeight ? (symbol.height + soffsetY - finalLineHeight) : 0;
					if (Yoffset > LastPicYoffset) {
						LastPicYoffset = Yoffset;
					}
				}

				vertsIndexList.Add (verts.size);
			...
			{
				//By Zsy : 当遍历到的字符不属于表情时，如果之前引用了bitmap字体，此处要先重置为普通字体状态计算
				if (bitmaplbl != null)
                {
					fontScale = 1;
                    useSymbols = false;
				...
				if (Mathf.RoundToInt(x + w) > regionWidth)
				{
					...
					//By Zsy : 这里LastPicYoffset如果不为零表示这一排有表情且与标准字体之间高度有差值，要把之前这一排记录的字体的顶点信息的Y坐标加以修正以实现下对齐
					if (LastPicYoffset != 0 && vertsIndexList.Count > 0) {
						for (int j = 0, jmax = vertsIndexList.Count; j < jmax; ++j) {
							Vector3 temp = verts [vertsIndexList [j]];
							temp.Set (temp.x, temp.y - LastPicYoffset - finalLineHeight + fontSize + soffsetY, temp.z);
				...
				if (!bold)
				{
					if (!italic)
					{
						//By Zsy : 出于时间和策划需求，这里就只考虑非加粗非斜体的字体，记录他们的顶点信息以便换行时统一处理高度
						vertsIndexList.Add (verts.size);
						verts.Add(new Vector3(v0x, v0y));
						vertsIndexList.Add (verts.size);
						verts.Add(new Vector3(v0x, v1y));
	...
	static public void PrintExactCharacterPositions (string text, BetterList<Vector3> verts, BetterList<int> indices, UILabelBitMap bitmaplbl = null)
	{
		if (string.IsNullOrEmpty(text)) text = " ";
