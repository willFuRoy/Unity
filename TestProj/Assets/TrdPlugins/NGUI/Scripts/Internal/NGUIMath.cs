...
static public class NGUIMath
{
	...
	static public Vector2 ConstrainRect (Vector2 minRect, Vector2 maxRect, Vector2 minArea, Vector2 maxArea, bool fixedtostart = false)
	{
...
//		if (!fixedtostart || (contentX >= areaX)) {
//			if (maxRect.x > maxArea.x)
//				offset.x -= maxRect.x - maxArea.x;
//		}
//		if (!fixedtostart || (contentY >= areaY)) {
//			if (maxRect.y > maxArea.y) 
//				offset.y -= maxRect.y - maxArea.y;
//		}
...
//		}
	...
	{  
		...
		{
			//By Zsy : 这里通常计算一个UIPanel的bounds，所以可以提前缓存UIPanel组件
			panel = content.GetComponent<UIPanel> ();   
			isSet = false;
			toLocal = relativeTo.worldToLocalMatrix;
	...
	static Dictionary<int, UIWidget> gWidgetsCache = new Dictionary<int, UIWidget>(10);
	[System.Diagnostics.DebuggerHidden]
	[System.Diagnostics.DebuggerStepThrough]
		...
		ref Matrix4x4 toLocal, ref Vector3 vMin, ref Vector3 vMax, ref bool isSet, bool considerChildren)
	{
		...
		UIPanel p = isRoot ? null : panel;
		if(p != null)
			p = panel.cachedTransform.Equals (content) ? p : null;
		...
		else // No panel present
		{
			...
			UIWidget w = null;
			//By Zsy : 这里之前为w = content.GetComponent<UIWidget>(),可以想象数量庞大时性能会降低很多.
			if (!gWidgetsCache.TryGetValue (content.GetInstanceID(), out w)) {
				w = content.GetComponent<UIWidget> ();
				gWidgetsCache.Add (content.GetInstanceID(), w);
			}

			if (w != null && w.enabled && !w.ignoreBounds)
			{
				Vector3[] corners = w.worldCorners;
	...
	/// <summary>
	/// Just CLear cache while game start!
	/// </summary>
    ...
    public static void ClearUnvalidRef() {
        // static member
        //gWidgetsCache.ClearUnvalidRefValue();
    }
}
