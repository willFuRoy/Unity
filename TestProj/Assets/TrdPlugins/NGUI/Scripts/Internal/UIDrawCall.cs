...
{
    public static bool gShowDrawCall = false;
	...
	[HideInInspector][System.NonSerialized] public List<UIWidget> widgets = new List<UIWidget>(20);
    ...
    static Shader _defShader;
    static Shader defaultShader {
        get {
            if(_defShader == null) {
                _defShader = Shader.Find("Unlit/Transparent Colored");
            }
            return _defShader;
        }
    }
	...
	void CreateMaterial ()
	{
        ...
        if (shader == null) {
            shader = oldShader != null ? oldShader : defaultShader;
        }
	...
	{
		if (!needbuild)
			needbuild = w.hasVertices && w.isVisible;
		widgets.Add (w);
	}
...
//					(tans.buffer != null && tans.buffer.Length != verts.buffer.Length);
	...
	const int maxIndexBufferCache = 50;
	...
	void SetClipping (int index, Vector4 cr, Vector2 soft, float angle)
	{
		...
		if (index < ClipRange.Length)
		{
			...
			clips.z = 1f / cr.z;
			clips.w = 1f / cr.w;
			mDynamicMat.SetVector(ClipRange[index], clips);

			clips.x = sharpness.x;
			clips.y = sharpness.y;
			clips.z = Mathf.Sin(angle);
			clips.w = Mathf.Cos(angle);
			mDynamicMat.SetVector(ClipArgs[index], clips);
		}
	}
    ...
    static public UIDrawCall Create (UIPanel panel, Material mat, Texture tex, Shader shader, bool grey)
	{
#if UNITY_EDITOR
		string name = null;
		...
		return Create(name, panel, mat, tex, shader, grey);
#else
		return Create(null, panel, mat, tex, shader, grey);
#endif
	}
	...
	static UIDrawCall Create (string name, UIPanel pan, Material mat, Texture tex, Shader shader, bool grey)
	{
	...
	static UIDrawCall Create (string name)
	{
        ...
        GameObject go = UnityEditor.EditorUtility.CreateGameObjectWithHideFlags(name,
            ...
            gShowDrawCall ? HideFlags.DontSave | HideFlags.NotEditable : HideFlags.HideAndDontSave, typeof(UIDrawCall));
    ...
    public static void RemoveDynamicMatCacheNullKey() {
