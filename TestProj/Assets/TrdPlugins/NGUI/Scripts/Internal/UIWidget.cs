...
public class UIWidget : UIRect
{
    ...
    // llm 2018-8-25 灰度相关
    [HideInInspector][SerializeField] protected bool mGrey = false;
    [Range(0.0f, 1.0f)]
    [HideInInspector][SerializeField] protected float mGreyWeight = 1.0f;
    // llm 2018-9-15 Table 相关 忽略进行 Bounds 计算
    [HideInInspector][SerializeField] protected bool mIgnoreBounds = false;
    //

    public delegate void OnDimensionsChanged (bool updateall);
    ...
    public delegate void OnAlphaChanged(float val);
    ...
    public OnAlphaChanged mOnAlphaChanged;
	...
	[System.NonSerialized] bool mAlphaChanged = false;
    ...
    /// 灰度缩放
    /// </summary>
    public float greyWeight {
        get {
            return mGreyWeight;
        }
        set {
            if (mGreyWeight != value) {
                mGreyWeight = value;
                if (mGrey) {
    ...
    public Color color
	{
		...
		set
		{
			if (mColor != value)
			{
				...
				gAlphaChanged = alphaChange;
				mColor = value;
				Invalidate(true, alphaChange);
				mAlphaChanged = true;
			}
		}
	}
	...
	public virtual Vector4 drawingDimensions
	{
		get
		{
            ...
            gDrawingDimensions.Set(
                mDrawRegion.x == 0f ? x0 : Mathf.Lerp(x0, x1, mDrawRegion.x),
                mDrawRegion.y == 0f ? y0 : Mathf.Lerp(y0, y1, mDrawRegion.y),
                mDrawRegion.z == 1f ? x1 : Mathf.Lerp(x0, x1, mDrawRegion.z),
                mDrawRegion.w == 1f ? y1 : Mathf.Lerp(y0, y1, mDrawRegion.w));

            return gDrawingDimensions;
        }
	}
    static Vector4 gDrawingDimensions;
	...
	protected void UpdateFinalAlpha (int frameID)
	{
		...
		{
			finalAlpha = 0f;
			filled = false;//mChanged ? false : (filled ? true : false);
		}
		else
		{
			UIRect pt = parent;
	...
	{
		mChanged = mChanged ? mChanged : includeChildren;
		mAlphaChanged = onlyAlpha;
		mAlphaFrameID = -1;
		...
		if (panel != null)
		{
			...
			if (includeChildren || mAlphaChanged) base.Invalidate(includeChildren, onlyAlpha);
		}
	}
	...
	static public int PanelCompareFunc (UIWidget left, UIWidget right)
	{
        ...
        // llm 灰度相关
        bool lgrey = left.grey;
        bool rgrey = right.grey;
        if (lgrey != rgrey) {
            return lgrey ? 1 : -1;
        }
        // 
	...
	protected override void OnAnchor ()
	{
		...
		newPos.Set(Mathf.Lerp(lt, rt, pvt.x), Mathf.Lerp(bt, tt, pvt.y), pos.z);
...
#endif
		{
			if (updateAnchors == AnchorUpdate.OnChange) {
//				UpdateAnchorsInternal (frame);
				mAnchorTargetChanged = true;
				UIRect.setDimension = true;
			}
		...
		if ((mMoved || mChanged) && onChange != null) onChange(false);
		return mMoved || mChanged;
	}

	//By Zsy : 当一个uiwidget只有alpha变化的时候，调用这个函数的重写来为具体ui处理alpha变化，而不是重建整个mesh
	protected virtual void FillAlpha()
	{
	...
	//by Zsy : filled这个变量用于当一个uiwidget只有alpha变化时，简单快速修改网格顶点alpha而不是重构网格，提高了效率降低了GC，但是当目标finalalpha为零时必须重构网格，具体原因请阅读代码
	bool filled = false;
...
#if UNITY_EDITOR
		//By Zsy : 这里用以保证编辑时能够实时相应变化
		if(!Application.isPlaying)
			mChanged = true;
#endif

		if (!mChanged && mAlphaChanged && filled) {
			FillAlpha ();
			if (mOnAlphaChanged != null)
				mOnAlphaChanged(finalAlpha);
		}
